...
| --- | --- | --- | --- | --- | --- |
| SPEC-0001 | [Contrato compartilhado de dados e análise](0001-shared-data-and-analysis-contract.md) | Baseline ativo v1.14; issue 0054 fechado com sunset bounded e apply final verificado; runtime Redis-free e maintenance-only no issue 0055; pré-disposição do storage retido no issue 0056, agora independente e pendente apenas dos gates de backup e alvo exato; boundaries estruturais 0032, 0033 e 0035; auditoria Redis 0037 | P0 / baseline | — | Fonte durável, integridade, contrato IA, ledger PostgreSQL de webhook, migrações, fronteiras de privacidade, runtime PostgreSQL-only e descarte Redis somente após backup e revisão exatos. |
...
`scripts.retire_ia_redis_compatibility`. Após a janela completa, o relatório
final de 2026-09-08 encontrou zero chaves em ambas as famílias e zero resultado
sem correspondência durável; o apply allowlisted deletou zero chaves porque elas
já haviam expirado. O importador histórico permanece no perfil `maintenance` e
a evidência é específica do checkout/runtime nomeado, não uma alegação de
produção ampla.
...
PostgreSQL ou workers. A janela de observação do issue 0054 foi concluída em
2026-09-08 e seu relatório final foi arquivado; por isso nenhum container ou
volume foi removido. A disposição
