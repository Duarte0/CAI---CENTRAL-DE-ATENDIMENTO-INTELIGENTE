...
- **Status:** implementada localmente pelo issue 0013; paginação completa é bloqueio apenas do full backfill
- **Versão:** 1.2
...
local sintética/descartável e não comprova provider, Redis ou produção. O full
backfill paginado permanece fora até validar o avanço de páginas do provider.
...
- `GET /contacts?perPage=40`, observado para listagem;
...
O checkout atual ainda não possui cliente Contacts, tabela de contato ou backfill desse diretório. O diretório em `src/core/digisac_directory.py` cobre somente departamentos e Users. O cliente DigiSac existente é, contudo, a autoridade de configuração e política de autenticação/retry a ser reutilizada ou estendida coerentemente.
...
5. Full backfill usa a listagem com `perPage=40`; somente pode ser ativado após validar a semântica de avanço suportada pelo provider. Não se pode inventar `page`, `offset`, cursor ou header. Até essa validação, persistência/upsert via webhook e hydration individual permanecem implementáveis, e refresh operacional deve limitar-se às chamadas cuja paginação esteja comprovada.
6. Quando o full backfill for ativado, página repetida, payload inválido ou avanço incompleto deve falhar a execução sem declará-la completa. Uma execução parcial nunca pode transformar ausência em exclusão ou inativação.
...
4. Doubles determinísticos devem provar snapshot de ticket, referência `contactId` sem busca por mensagem, hydration deduplicada, grupo, replay, precedência e preservação diante de dados sem ordenação. Após a paginação ser comprovada, devem cobrir backfill completo e falha parcial.
...
- O slice de migration/modelo/upsert de ticket/hydration individual, com os testes acima, **pode abrir issue de implementação agora**. A issue deve declarar explicitamente que full backfill paginado fica fora do slice até comprovar o mecanismo de paginação Contacts.

## Decisão residual mínima

A única lacuna de provider que bloqueia parte material desta SPEC é a semântica completa de avanço da paginação de `GET /contacts?perPage=40`; ela bloqueia somente o full backfill/reconciliação que dependa de múltiplas páginas. Não bloqueia modelagem, persistência, upsert incremental ou hydration individual. Não há decisão de produto pendente para este slice.
