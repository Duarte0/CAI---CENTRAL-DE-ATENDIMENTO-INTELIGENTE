"""Inventory and boundedly retire legacy audio Redis lists.

The audio worker polls PostgreSQL directly. This command never republishes work:
it compares bounded Redis-list snapshots with durable transcription rows and, in
...
LEGACY_AUDIO_QUEUE = "audio_transcription_queue"
LEGACY_AUDIO_DEAD_LETTER = "audio_transcription_dead_letter"
DEFAULT_MAX_ITEMS = 1_000
_APPLY_CONFIRMATION = "retire-legacy-audio-queue"


class LegacyAudioRedis(Protocol):
...
TranscriptionLookup = Callable[
    [list[str]], Awaitable[dict[str, dict[str, Any]]]
]


@dataclass(frozen=True)
class LegacyAudioInventory:
...
async def inventory_legacy_audio_lists(
    redis: LegacyAudioRedis,
    *,
    transcription_lookup: TranscriptionLookup,
    max_items: int = DEFAULT_MAX_ITEMS,
) -> LegacyAudioInventory:
    """Build a bounded inventory without mutating Redis or PostgreSQL."""
    if max_items <= 0:
        raise ValueError("max_items must be positive")
    queues = (LEGACY_AUDIO_QUEUE, LEGACY_AUDIO_DEAD_LETTER)
    ...
    durable_rows = await transcription_lookup(message_ids)
    entries_by_message: dict[str, dict[str, int]] = {}
    unknown_counts: dict[str, int] = {}
    ...
    for key in queues:
        ...
        for _value, message_id in parsed[key]:
            counts = entries_by_message.setdefault(
                message_id, {LEGACY_AUDIO_QUEUE: 0, LEGACY_AUDIO_DEAD_LETTER: 0}
            )
            counts[key] += 1
...
async def retire_validated_legacy_audio_entries(
    redis: LegacyAudioRedis,
    inventory: LegacyAudioInventory,
) -> int:
...
async def recover_legacy_transient_dead_letters(
    ...
    """Import persisted transient dead-letter evidence into DB-only retry state."""
    recovered = 0
    for message_id, row in inventory.durable_rows.items():
        ...
        retry_at = datetime.now(timezone.utc) + timedelta(
            seconds=settings.audio_retry_base_seconds
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Inventory legacy audio Redis lists and optionally import transient "
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        await redis.ping()
        inventory = await inventory_legacy_audio_lists(
            redis,
            transcription_lookup=_lookup_transcriptions,
        ...
        if args.apply:
            report["removed_validated_entries"] = (
                await retire_validated_legacy_audio_entries(redis, inventory)
            )
        return report
    finally:
